...
exports.updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  if (req.io) {
    ...
    // 🔐 Restaurant dashboard (private)
    req.io.to(`restaurant:${username}`).emit("order:updated", payload);

    // 🌍 Public users (order status page)
    req.io.to(`public:restaurant:${username}`).emit("order:updated", payload);

    console.log(`✅ Order ${orderId} status updated to ${status} - broadcasted to both rooms`);
  }
...
exports.deleteOrder = asyncHandler(async (req, res, next) => {
  ...
  if (req.io) {
    ...
    // 🔐 Restaurant dashboard (private)
    req.io.to(`restaurant:${username}`).emit("order:deleted", payload);

    // 🌍 Public users (order status page)
    req.io.to(`public:restaurant:${username}`).emit("order:deleted", payload);

    console.log(`✅ Order ${orderId} deleted - broadcasted to both rooms`);
  }
