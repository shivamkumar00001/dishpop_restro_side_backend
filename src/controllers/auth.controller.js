...
const register = catchAsyncError(async (req, res, next) => {
  ...
  // Create user with phone as first delivery number
  const user = await Owner.create({
    username,
    restaurantName,
    ownerName,
    email,
    phone,
    deliveryPhones: [{ number: phone, label: "Primary", isPrimary: true }],
    state,
    city,
    pincode,
    restaurantType,
...
const updateProfile = catchAsyncError(async (req, res, next) => {
  ...
  // 🔥 Handle delivery phones (parse from JSON string)
  if (req.body.deliveryPhones !== undefined) {
    try {
      const deliveryPhones = JSON.parse(req.body.deliveryPhones);
      if (Array.isArray(deliveryPhones)) {
        ...
        user.deliveryPhones = deliveryPhones;
      }
    } catch (error) {
      return next(new ErrorHandler("Invalid delivery phones format", 400));
    }
  }
...
const getLandingPageData = catchAsyncError(async (req, res, next) => {
  ...
  const user = await Owner.findOne({ username }).select(
    "restaurantName ownerName profilePhoto galleryImages description restaurantType city state deliveryPhones"
  );
  ...
  console.log("✅ Found restaurant:", user.restaurantName);
  console.log("📸 Gallery images count:", user.galleryImages?.length || 0);
  console.log("📞 Delivery phones count:", user.deliveryPhones?.length || 0);
  ...
  const responseData = {
    ...
    city: user.city,
    state: user.state,
    deliveryPhones: user.deliveryPhones || [],
    rating: avgRating,
    reviewCount,
    dishCount,
    stats: {
